...
class ModelServiceCustomer extends Model {
		...
		public function getCustomer($custcode) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX_APP . "cust WHERE CustCode = '" . $custcode . "'");

			return $query->row;
		}

		public function getCustKey($custcode) {
			$query = $this->db->query("SELECT CustName1 FROM " . DB_PREFIX_APP . "cust WHERE CustCode = '" . $custcode . "'");
		...
		public function getCustomerContacts($data = array()) {
			$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.custcode) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		public function getTotalCustomerContacts($data = array()) {
		...
		public function getTotalCustomers($data = array()) {
			$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX_APP . "cust";
