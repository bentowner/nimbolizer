...
public function getCustomerGroups($data = array()) {
		$sql = "SELECT *,cg.customer_group_id as customer_group_id FROM " . DB_PREFIX . "customer_group cg LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (cg.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (isset($data['customer_group_id']) && !is_null($data['customer_group_id'])) {
			$sql .= " and cg.customer_group_id = '". $data['customer_group_id'] ."' ";
		} else {
		...
		$this->session->data['test_groups'] =$sql;
		return $query->rows;
	}
