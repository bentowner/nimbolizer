...
class ModelLocalisationOrderStatus extends Model {
	...
	public function getBookingStatuses($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "booking_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'";
			...
			$booking_status_data = $this->cache->get('booking_status.' . (int)$this->config->get('config_language_id'));
	
			if (!$booking_status_data) {
				$query = $this->db->query("SELECT BKSCode, name FROM " . DB_PREFIX . "booking_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
	
				$booking_status_data = $query->rows;
	
				$this->cache->set('booking_status.' . (int)$this->config->get('config_language_id'), $booking_status_data);
			}
	
			return $booking_status_data;
		}
