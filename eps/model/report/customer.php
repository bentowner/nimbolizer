...
class ModelReportCustomer extends Model {
	...
	public function getSalesRep($data = array()){
		$sql = "select custrlcl,custlcom,SRName,srtype from lcl_cust as cust 
				inner join lcl_salesrep as sales on sales.SRCode = cust.custrlcl 
				inner join troy_customer_commission_type as comm on comm.srtcode = cust.custlcom";

		if (!empty($data['filter_customer_code'])) {$sql .= " where custcode = '" . $data['filter_customer_code'] . "'";}
	...
	public function getTotalBookings($data = array()) {
		$sql = "SELECT COUNT(DISTINCT book.bkref) AS total FROM `" . DB_PREFIX_APP . "book` book ";
		
		if (!empty($data['filter_customer_id'])) {
		 	$sql .= " WHERE book.bkcust = '" . $data['filter_customer_id'] . "'";
		} else {
			$sql .= " WHERE book.bkcust > '0' ";
		}
		
		if (!empty($data['filter_valid'])) {$sql .= " and book.bkstat !='N' and book.bkstat !='H' ";}

		if (!empty($data['filter_day_ago'])) {$sql .= "and book.bkdate >= date_add(Now(),interval '" . $this->db->escape($data['filter_day_ago']) ."' day) ";}
	...
	public function getTotalQuotes($data = array()) {
		...
		$sql = "SELECT COUNT(DISTINCT quote.qtnumb) AS total FROM `" . DB_PREFIX_APP . "quotes` quote ";
	
		if (!empty($data['filter_customer_id'])) {
			$sql .= " WHERE quote.qtcust = '" . $data['filter_customer_id'] . "'";
		} else {
			$sql .= " WHERE quote.qtcust > '0' ";
		}
	
		if (!empty($data['filter_valid'])) {$sql .= " and quote.qtdisp !='N' and quote.qtdisp !='H' ";}
	
		if (!empty($data['filter_day_ago'])) {$sql .= "and quote.qtdate >= date_add(Now(),interval '" . $this->db->escape($data['filter_day_ago']) ."' day) ";}
