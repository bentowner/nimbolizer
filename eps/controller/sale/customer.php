...
class ControllerSaleCustomer extends Controller {
	...
	public function index() {
		...
		$this->session->data['link'] =$data['link'] =$this->url->link('service/booking/update', 'token=' . $this->session->data['token'] . '&booking_no=', 'SSL');
	...
	public function add_shipper() {
		...
		if (isset($this->request->get['filter_customer_group_id'])) {
			$this->request->post['customer_group_id']=$this->request->get['filter_customer_group_id'];
		}
		
		$this->model_sale_customer->addCustomer_shipper($this->request->post);
		...
		$this->session->data['success'] = $this->language->get('text_success');
			...
			if($this->request->get['Use'] && $this->request->get['Use']=='Quote'){
				
			$this->response->redirect($this->url->link('service/quotations/insert', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			}else if($this->request->get['Use'] && $this->request->get['Use']=='Booking'){
				
			$this->response->redirect($this->url->link('service/booking/insert', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			}else {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($this->session->data['customer']){$this->request->post['customer_group_id'] = $this->session->data['customer'];	}
			if($this->request->get['filter_customer_group_id']){$this->request->post['customer_group_id'] = $this->request->get['filter_customer_group_id'];	}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->session->data['link'] = $data['link'] =$this->url->link('service/quotations/update', 'token=' . $this->session->data['token'] . '&quote_no=', 'SSL');
			$this->response->redirect($this->url->link('service/quotations/insert', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			}else if($this->request->get['Use'] && $this->request->get['Use']=='Booking'){
			$this->session->data['link'] =$data['link'] =$this->url->link('service/booking/update', 'token=' . $this->session->data['token'] . '&booking_no=', 'SSL');
	...
	public function edit_shipper() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			if(isset($this->session->data['customer_group_id'])){
				$this->request->post['customer_group_id'] = $this->session->data['customer_group_id'];
			}
			
			$this->model_sale_customer->editCustomer_shipper($this->request->get['shipper_id'], $this->request->post);
	...
	protected function getList() {
		...
		if (isset($this->request->get['Use'])){$data['Use'] = $this->request->get['Use'];} else {$data['Use'] = '';	}
		...
		$customer_total = $this->model_service_customer->getTotalCustomerContacts($filter_data);

		$results = $this->model_service_customer->getCustomerContacts($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'approve'        => $approve,
				'unlock'         => $unlock,
				'edit'           => $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_group_id='.$result['customer_group_id'] .'&custcode='.$result['custcode']  . $url, 'SSL')
			);
		}
	...
	protected function getForm() {
		...
		$data['entry_shipper'] = $this->language->get('entry_shipper');
		...
		$data['token'] = $this->session->data['token'];
		if (isset($this->request->get['Use'])){$data['Use'] = $this->request->get['Use'];} else {$data['Use'] = '';	}
		if (isset($this->request->get['shipper'])){$data['shipper'] = $this->request->get['shipper'];} else {$data['shipper'] = '';	}
		
		if(isset($this->session->data['customer'])){
			
			$data['customer_group_id'] = $this->session->data['customer'];	
		...
		if (isset($this->error['agent'])) {
			$data['error_agent'] = $this->error['agent'];
		} else {
			$data['error_agent'] = '';
		}
		...
		if (isset($this->request->get['Use'])) {
			$url .= '&Use=' .$this->request->get['Use'];
		}
		if (isset($this->request->get['shipper'])) {
			$url .= '&shipper=' .$this->request->get['shipper'];
		}
		if (isset($this->request->get['shipper_id'])) {
			$url .= '&shipper_id=' .$this->request->get['shipper_id'];
		}
		if (isset($this->request->get['filter_name'])) {
			$url .= '&filter_name=' . urlencode(html_entity_decode($this->request->get['filter_name'], ENT_QUOTES, 'UTF-8'));
		}
		...
		if(isset($this->request->get['shipper']) && $this->request->get['shipper']=='true'){
			if (!isset($this->request->get['shipper_id'])) {
			$data['action'] = $this->url->link('sale/customer/add_shipper', 'token=' . $this->session->data['token'] . $url, 'SSL');
			} else{
			$data['action'] = $this->url->link('sale/customer/edit_shipper', 'token=' . $this->session->data['token'].'&shipper_id=' . $this->request->get['shipper_id']  . $url, 'SSL');
			}
		...
		if (isset($this->request->get['shipper_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$customer_info = $this->model_sale_customer->getCustomer_shipper($this->request->get['shipper_id']);
		}
		...
		if (isset($this->request->get['Use'])) {
			$data['Use'] = $this->request->get['Use'];
		}
		
		
		if (!empty($customer_info)) {
			$data['customer_group_id'] = $this->session->data['customer_group_id'] = $customer_info['customer_group_id'];
		} else {
			$data['customer_group_id'] ='';
					
		}
		
		if (isset($this->request->get['custcode'])) {
			$data['custcode'] = $this->request->get['custcode'];
		} elseif (!empty($customer_info) && $customer_info['custcode']) {
			$data['custcode'] = $customer_info['custcode'];
		} else {
			$data['custcode'] = 0;
		}
		...
		if (!empty($customer_info) && isset($customer_info['customer_agent_id'])) {
			$data['customer_agent_id'] = $customer_info['customer_agent_id'];
			if (isset($customer_info['customer_agent_id']) && !empty($customer_info['customer_agent_id'])){
				$data['agent'] = $this->model_sale_customer->getAgentName($customer_info['customer_agent_id']);
			}else {
				$data['agent'] ='';
			}
			
		} else {
			$data['customer_agent_id'] = '';
			$data['agent'] ='';
		}
	...
	/*
		if (isset($this->request->post['company'])){
			if ((utf8_strlen($this->request->post['company']) < 1) || (utf8_strlen(trim($this->request->post['company'])) > 32)) {
	...
	*/	
		...
		/**
		 * @todo Business decision to not include lastname for expediency...
		...
		*/
		...
		/**
		 * @todo Business decision to not include restrictions of multiple email addresses for expediency...
		...
		if (!isset($this->request->get['customer_id'])) {
			...
			if ($customer_info && ($this->request->get['customer_id'] != $customer_info['customer_id'] && $this->request->post['customer_group_id'] != $customer_info['customer_group_id'])) {
				$this->error['warning'] = $this->language->get('error_exists');
			}
		}
		*/
		...
		// ONLY require phone number for new contact
		if ((utf8_strlen($this->request->post['telephone']) < 3) || (utf8_strlen($this->request->post['telephone']) > 32)) {
			if(!isset($this->request->get['customer_id'])){$this->error['telephone'] = $this->language->get('error_telephone');}
		}
		...
		*/
		if (!isset($this->request->post['password'])){$this->request->post['password'] = 'troy';
		$this->request->post['confirm']='troy';
		}

		if ($this->request->post['password'] || (!isset($this->request->get['customer_id']))) {
			if ((utf8_strlen($this->request->post['password']) < 4) || (utf8_strlen($this->request->post['password']) > 20)) {
				$this->request->post['password']='troy';
	...
	public function transaction() {
		...
		$data['book_link'] = $this->url->link('service/booking/update', 'token=' . $this->session->data['token'] . '&booking_no=', 'SSL');
		$data['token'] = $this->session->data['token'];
	...
	public function getContactList(){
		...
		$json = array();
		if (isset($this->request->get['customer'])){$this->session->data['customer'] = $this->request->get['customer'];		}
	
		if(isset($this->session->data['customer']) && !empty($this->session->data['customer'])){
				$customer = $this->session->data['customer'];
			if(isset($data['customer']) && !empty($data['customer'])){
				$customer = $data['customer'];
			}
			if (isset($this->request->get['customer'])){
				$this->session->data['customer'] = $this->request->get['customer'];
			}
			$this->load->model('sale/customer');
			
			$results = $this->model_sale_customer->getCustomers(array('filter_custcode' =>  $this->session->data['customer']));
			
			
				foreach ($results as $result) {
					$contact_stats = $this->model_sale_customer->sumCustomer(array('filter_customer_id' =>  $result['customer_id']));
					
					$action = array();
					
					if (isset($this->session->data['ActiveSession']) && $this->session->data['ActiveSession'] == 'Quote'){
						$action[] = array(
								'text' 		=> 'Use this Contact',
								'edit'		=> $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token']. '&customer_id=' . $result['customer_id'] .'&filter_customer_group_id='. $result['Cust_Code'].'&Use=Quote', 'SSL'),
								'href'      => $this->url->link('service/quotations/addcontact', 'token=' . $this->session->data['token']. '&customer_id=' . $result['customer_id'], 'SSL')
						);
					} else {
						$action[] = array(
								'text' 		=> 'Use this Contact',
								'edit'		=> $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token']. '&customer_id=' . $result['customer_id'] .'&filter_customer_group_id='. $result['Cust_Code'].'&Use=Booking', 'SSL'),
								'href'      => $this->url->link('service/booking/addcontact', 'token=' . $this->session->data['token']. '&customer_id=' . $result['customer_id'], 'SSL')
						);
					}
					...
					$json[] = array(
							...
							'recent'			=> $contact_stats['recent']
							
					);
					
					
					$this->session->data['customer_group_id']= $result['customer_group_id'];
				}
				...
				if (sizeof($json)>1){
					...
					foreach ($json as $key => $row) {
						...
						$total[$key]	= $row['total'];
						$recent[$key]	= $row['recent'];
					}
						
					array_multisort($total, SORT_DESC, $recent, SORT_ASC, $json);
		...
		public function getShipperList(){
			...
			$results = $this->model_sale_customer->getShippers(array('filter_custcode' =>  $this->session->data['customer']));
			
			foreach ($results as $result) {
				$contact_stats = $this->model_sale_customer->sumShipper(array('filter_shipper_id' =>  $result['customer_id']));
				$this->session->data['agent_contact_info'] =$agent_info = $this->model_sale_customer->getCustomer($result['customer_agent_id']);
				$agentName ='';$agentEmail='';$agentId='000000';
				if(isset($agent_info['firstname'])){
					$agentName = $agent_info['firstname'];	
				}
				
				if(isset($agent_info['lastname'])){
					$agentName .= ' '.$agent_info['lastname'];
				}
				if(isset($agent_info['email'])){
					$agentEmail=$agent_info['email'];
				}
				if(isset($agent_info['customer_id'])){
					$agentId=$agent_info['customer_id'];
				}
				
				
				$json[] = array(
						'shipper_id'        => $result['customer_id'],
						'shipper_company'   => $result['company'],
						'shipper_contact'   => $result['contact'],
						'shipper_email'     => $result['email'],
						'customer_group_id'	=> $result['customer_group_id'],
						'customer_agent'	=> $agentName,
						'customer_agent_email'=>$agentEmail,
		...
		public function getAgentList(){
			if (isset($this->request->get['custcode'])){$this->session->data['customer'] = $this->request->get['custcode'];		}
			if(isset($this->session->data['customer']) && !empty($this->session->data['customer'])){
				if (isset($this->request->get['filter_name'])) {
				$this->load->model('sale/customer');
				...
				$data = array(
						'filter_name' => $this->request->get['filter_name'],
						'filter_custcode' =>  $this->session->data['customer'],
						'start'       => 0,
						'limit'       => 20
				);
