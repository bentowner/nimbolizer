  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right">
       ...
       <a type="button" id="button-booking-view" data-toggle="tooltip" href="<?php echo $bookingsearch; ?>" title="<?php echo $button_booking_view; ?>" class="btn btn-info"><i class="fa fa-flag"></i></a>
   ...
   <?php if ($bookingnumber && $bookingnumber !=null) { ?>    
       <a type="button" id="button-print" form="form-booking" href="<?php echo $print ?>" TARGET="_blank" data-toggle="tooltip" title="<?php echo $button_booking_print; ?>" class="btn btn-info"><i class="fa fa-print"></i></a>
  <?php } ?> 
       <a type="button" id="button-quotes-view" form="form-booking" href="<?php echo $quotesearch; ?>" data-toggle="tooltip" title="<?php echo $button_quotes_view; ?>" class="btn btn-info"><i class="fa fa-flag-o"></i></a>
       <a type="button" id="button-quotes-web-view" form="form-booking" href="<?php echo $webquotesearch; ?>" data-toggle="tooltip" title="<?php echo $button_quotes_web_view; ?>" class="btn btn-info"><i class="fa fa-external-link"></i></a>
  <?php if ($truckingshipid && $truckingshipid !=null) { ?>
       <a type="button" id="button-save"  data-toggle="tooltip"  data-loading-text="<?php echo $text_loading; ?>" title="<?php echo $button_cts_info; ?>" class="btn btn-primary cts"  ><i class="fa fa-check-circle"></i> <?php echo $button_cts; ?></a>     
  <?php } ?>
...
<!--- // Rates calc or entered // --->            
 ...
 <script type="text/javascript">$(document).ready(function() {
	 var url_reload= 'index.php?route=service/booking/insert&token=<?php echo $token; ?>';
	 ...
	 shortcut.add("Ctrl+Shift+2",function() {
		...
		if (typeof booking_object == 'undefined'){	
			...
			var bookingnumber = input_booking_object["bookingnumber"];
			var booking = Object.keys(input_booking_object);
			var vals = Object.keys(input_booking_object).map(function (key) {  return input_booking_object[key];});
			var clone_booking = toObject(booking,vals);
			var clean = removeUnique();
			//var finish = refresh();
			alert('You just Cloned Booking number'+bookingnumber+'! Press F5 to load it.');
		}
	 ...
	 function removeUnique(){
		 $.post('controller/setting/ajaxpersist.php', { 'fieldname' :  'bookingnumber', 'value': null});
		 $.post('controller/setting/ajaxpersist.php', { 'fieldname' :  'bookingdate', 'value': null});
		 $.post('controller/setting/ajaxpersist.php', { 'fieldname' :  'bookedby', 'value': null});
		 $.post('controller/setting/ajaxpersist.php', { 'fieldname' :  'status', 'value': null});
		 $.post('controller/setting/ajaxpersist.php', { 'fieldname' :  'BkStat', 'value': null});
...
<script type="text/javascript">$(document).ready(function() {
		...
		if (destination_service==false){$("#destination_service_group").removeAttr("style").hide();} else if (destination_service==true){$("#destination_service_group").removeAttr("style").show();	$("#rates_oncarriage_group").removeAttr("style").show();$("#pointofdelivery").removeAttr("style").show();}
...
<script type="text/javascript"><!--
			...
			$('#tracking').load('index.php?route=service/booking/getHistory&token=<?php echo $token; ?>');
			
			$('#button-tracking').on('click', function(e) {
			  e.preventDefault();
			
				$.ajax({
					url: 'index.php?route=service/booking/getHistory&token=<?php echo $token; ?>',
					type: 'post',
					dataType: 'html',
...
//--></script>
 ...
 <script type="text/javascript">$(document).on("keydown keyup", function() {
	...
	$(".dims").change("keydown keyup", function() {calculateDim();	});  
 ...
 <script type="text/javascript">
	...
	function addDimension() {
		...
		html += '  <td class="text-left"><input type="text" class="form-control length" /></td>';
		html += '  <td class="text-left"><input type="text" class="form-control width" /></td>';
		html += '  <td class="text-left"><input type="text" class="form-control height" /></td>';
		html += '  <td class="text-right"> </td>';
		html += '  <td class="text-left"><button type="button" onclick="$(\'#dim-row' + dim_row + '\').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
		html += '</tr>';
...
$('#history').load('index.php?route=service/booking/getHistory&token=<?php echo $token; ?>&booking_no=<?php echo $bookingnumber; ?>');

$('#button-history').bind('click', function() {
	$.ajax({
		url: 'index.php?route=service/booking/getHistory&token=<?php echo $token; ?>&booking_no=<?php echo $bookingnumber; ?>',
		type: 'post',
		dataType: 'html',
...
$(document).on("click", ".sail", function(e) {

	var sail = <?php echo $sailing; ?>;
	var url= 'index.php?route=maintenance/vessels&token=<?php echo $token; ?>&vessel_alt=true';		
	bootbox.dialog({
		message: sail,
		title: "Vessel Sailing Schedule",
		
		buttons: {
			success: {label: "Change Service",className: "btn-danger",callback: function() {
				alert('Confirm Changing to a vessel in Another Service');	
...
<script type="text/javascript"><!--
	...
	$(document).on("click", ".cts", function(e) {
		...
		var contact = <?php echo $CTS_order; ?>;
		var url_w_cts= 'index.php?route=service/booking/insertBooking&token=<?php echo $token; ?>&sendCTS=true';	
		var url= 'index.php?route=service/booking/insertBooking&token=<?php echo $token; ?>';	
			
		bootbox.dialog({
			message: contact,
			title: "Create Booking with CTS ",
			
			buttons: {
				success: {
					label: "Proceed Book it.",className: "btn-primary",callback: function() {
					window.location.href = url_w_cts ;
	
				}},
				skip: {
					label: "Save without CTS.",className: "btn-primary",callback: function() {
					window.location.href = url ;
...
$(document).on("click", ".wsa", function(e) {
	...
	var url= 'index.php?route=service/booking/updateRates&token=<?php echo $token; ?>';	
	var options =[{
...
<script type="text/javascript">$(document).ready(function() {var rates_precarriage = $("input[name=rates_precarriage]").val();
	...
	$.post('controller/setting/ajaxpersist.php', {'fieldname' : 'rates_precarriage_amount','value':rates_precarriage_min2.toFixed(2)} );
	$('input[name=\'rates_precarriage_amount\']').val(rates_precarriage_min2.toFixed(2));
	});
